...
CRENDEROBJECT           g_RenderObject_RTT;         // 显示的物体
...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
  {
    ...
    DXUTCreateWindow( L"EmptyProject_DXUT_2_DetectEdgeInPixel" );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    lstrcat( szRcPath ,L"\\ResFile_1_RTT" );
    SetCurrentDirectory( szRcPath );
    TCHAR* szFile_1 = L"ShmFile.shm";
    if( FAILED( hr = g_RenderObject_RTT.OnCreateDevice( pd3dDevice ,szFile_1 ))) return hr;
    //
    lstrcpy( szRcPath ,szAppPath );
...
HRESULT CALLBACK OnResetDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    //
    g_RenderObject_RTT.OnResetDevice( pd3dDevice , pBackBufferSurfaceDesc->Width , pBackBufferSurfaceDesc->Height );
    g_RenderObject_Ground.OnResetDevice( pd3dDevice , pBackBufferSurfaceDesc->Width , pBackBufferSurfaceDesc->Height );
...
void CALLBACK OnFrameMove( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    g_RenderObject_RTT.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime  );
    g_RenderObject_Ground.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime  );
    // 将物体渲染到纹理
    vEyePt.x  = vEyePt.x  ;
    vEyePt.y  = vEyePt.y  ;
    g_RenderObject_RTT.RenderToTexture( pd3dDevice , vEyePt ,fTime );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene( )))
      {
        g_RenderObject_RTT.OnFrameRender( pd3dDevice );
        g_RenderObject_Ground.OnFrameRender( pd3dDevice );
        
        g_TextureDebug.OnFrameRender( pd3dDevice ,g_RenderObject_RTT.ReturnTexture( ));
...
void CALLBACK OnLostDevice()
  {
    ...
    g_RenderObject_RTT.OnLostDevice( );
...
void CALLBACK OnDestroyDevice()
  {
    ...
    g_RenderObject_RTT.OnDestroyDevice( );
