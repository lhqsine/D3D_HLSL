...
sampler ColorMapSampler_5 = sampler_state
  {
    Texture   = ( texColorTexture_5 );
    MipFilter = LINEAR;
    MagFilter = LINEAR;
...
VS_OUTPUT MainVS_Screen_5( VS_INPUT In )
  {
    ...
    Out.pos  = float4( In.pos.x , In.pos.y , 0.999 , 1 );
    Out.tex       = In.tex;
    Out.color = In.color;
    return Out;
  }
...
float4 MainPS_Screen_5( VS_OUTPUT In ):COLOR
  {
    float4 vColor = tex2D( ColorMapSampler_5 ,In.tex ) ;
    return vColor;
  }
    ...
    pass P0
      {
        ...
        */
        ...
        StencilFunc      = Always;
        StencilRef       = 0x0f;
        StencilPass      = Replace;//Incrsat;  // 通过测试，模板缓冲区加 1
