...
HRESULT CBOARD::OnResetDevice( IDirect3DDevice9* pd3dDevice , int iWidth , int iHeight )
  {
    HRESULT hr = S_OK;
    if( FAILED( hr = CreateSquare( pd3dDevice ))) return hr;
    hr = m_pEffect->SetTexture( "texColorTexture" ,m_pColorTexture );
    D3DXVECTOR4 v4ScreenSize = D3DXVECTOR4( iWidth , iHeight , 0.0 , 0.0 );
    hr = m_pEffect->SetVector( "g_v4ScreenSize" , &v4ScreenSize );
    return hr;
  }
