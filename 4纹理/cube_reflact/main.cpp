...
//CRENDEROBJECT           g_RenderObject;         // 坐标物体
CRENDEROBJECT           g_ReflectObject;         // 反射物体
CBOXOBJECT              g_Box;                    // 立方体纹理
...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
  {
    ...
    DXUTCreateWindow( L"EmptyProject_DXUT_8_Reflect" );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    */
    ...
    if( FAILED( hr = g_Box.OnCreateDevice( pd3dDevice ,szFile_2 ))) return hr;
    // 
    lstrcpy( szRcPath ,szAppPath );
    lstrcat( szRcPath ,L"\\ResFile_3_ReflectObject" );
    SetCurrentDirectory( szRcPath );
    TCHAR* szFile_3 = L"ShmFile.shm";
    if( FAILED( hr = g_ReflectObject.OnCreateDevice( pd3dDevice ,szFile_3 ))) return hr;
    // 
    return S_OK;
  }
...
HRESULT CALLBACK OnResetDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    D3DXMatrixPerspectiveFovLH( &g_matProject ,D3DX_PI/4 ,fAspectRatio ,1.0 ,800.0 );
...
void CALLBACK OnFrameMove( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    g_ReflectObject.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime );
    
    g_ReflectObject.SetViewVector( &vEyePt );
    //D3DXMatrixIdentity( &g_matWorld );
    g_Box.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime );
  }
