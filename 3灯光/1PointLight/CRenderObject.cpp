...
HRESULT CRENDEROBJECT::OnCreateDevice( IDirect3DDevice9* pd3dDevice ,TCHAR* FileName )
  {
    ...
    if( FAILED( hr = D3DXLoadMeshFromX( szFile ,D3DXMESH_SYSTEMMEM ,
      pd3dDevice, NULL, &pBuffer , NULL, &m_dwMaterial , &pMeshSysMemClone ))) return hr;
    //--------------------------- 处理材质 ----------------------------------------
    if( NULL == ( m_pMaterial = new D3DXMATERIAL[ m_dwMaterial ] )) return E_FAIL;
...
HRESULT CRENDEROBJECT::OnResetDevice( )
  {
    ...
    if( FAILED( hr = m_pEffect->SetValue( "g_Material" , m_pMaterial , sizeof( D3DXMATERIAL )))) return hr;
    return hr;
  }
