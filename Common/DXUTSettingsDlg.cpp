...
void CD3DSettingsDlg::CreateControls()
  {
    ...
    m_Dialog.SetFont( 0, L"宋体", 15, FW_NORMAL );
    m_Dialog.SetFont( 1, L"宋体", 28, FW_BOLD );
    ...
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"Direct3D 设置", 10, 5, 400, 50, false, &pStatic );
    pElement = pStatic->GetElement( 0 );
    pElement->iFont = 1;
    pElement->dwTextFormat = DT_TOP | DT_LEFT;
    // 选择显示卡
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"显示卡", 10, 40, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_ADAPTER,        200, 40, 300, 23 );
    // 选择设备类型
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"着色设备", 10, 65, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_DEVICE_TYPE, 200,  65, 300, 23 );
    // 全屏、窗口模式
    m_Dialog.AddRadioButton( DXUTSETTINGSDLG_WINDOWED, DXUTSETTINGSDLG_WINDOWED_GROUP, L"窗口模式", 240, 95, 300, 16 );
    m_Dialog.AddCheckBox( DXUTSETTINGSDLG_DEVICECLIP, L"但超过显示器时剪裁", 350, 110, 400, 16 );
    m_Dialog.AddRadioButton( DXUTSETTINGSDLG_FULLSCREEN, DXUTSETTINGSDLG_WINDOWED_GROUP, L"全屏模式", 240, 130, 300, 16 );
    // 显示格式
    m_Dialog.AddStatic( DXUTSETTINGSDLG_ADAPTER_FORMAT_LABEL, L"显示卡格式", 10, 150, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_ADAPTER_FORMAT,                   200, 150, 300, 23 );
    // 分辨率
    m_Dialog.AddStatic( DXUTSETTINGSDLG_RESOLUTION_LABEL, L"分辨率", 10, 175, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_RESOLUTION,               200, 175, 300, 23 );
    m_Dialog.GetComboBox( DXUTSETTINGSDLG_RESOLUTION )->SetDropHeight( 106 );
    // 刷新率
    m_Dialog.AddStatic( DXUTSETTINGSDLG_REFRESH_RATE_LABEL, L"刷新率", 10, 200, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_REFRESH_RATE,               200, 200, 300, 23 );
    // 背景缓冲区格式
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"背景缓冲区格式", 10, 225, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_BACK_BUFFER_FORMAT,     200, 225, 300, 23 );
    // 深度模板缓冲区格式
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"深度/模板缓冲区格式", 10, 250, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_DEPTH_STENCIL,              200, 250, 300, 23 );
    // 多采用类型
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"多采样类型", 10, 275, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_MULTISAMPLE_TYPE, 200, 275, 300, 23 );
    // 多采用质量
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"多采样质量", 10,  300, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_MULTISAMPLE_QUALITY, 200, 300, 300, 23 );
     // DXUTSETTINGSDLG_VERTEX_PROCESSING
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"顶点处理方式", 10, 325, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_VERTEX_PROCESSING, 200, 325, 300, 23 );
     // DXUTSETTINGSDLG_PRESENT_INTERVAL
    m_Dialog.AddStatic( DXUTSETTINGSDLG_STATIC, L"切换方式",  10, 350, 180, 23 );
    m_Dialog.AddComboBox( DXUTSETTINGSDLG_PRESENT_INTERVAL, 200, 350, 300, 23 );
    // DXUTSETTINGSDLG_OK, DXUTSETTINGSDLG_CANCEL
    m_Dialog.AddButton( DXUTSETTINGSDLG_OK, L"确认",     240, 375, 73, 31 );
    m_Dialog.AddButton( DXUTSETTINGSDLG_CANCEL, L"取消", 325, 375, 73, 31, 0, true );
  }
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
void WINAPI CD3DSettingsDlg::StaticOnEvent( UINT nEvent, int nControlID,  CDXUTControl* pControl )
  {
    if( g_pD3DSettings )
