...
MOTION_STATUS RK4( MOTION_STATUS* Status0 , double Time ,float h )
  {
    ...
    //
    Status.v3Position = Status0->v3Position + Status0->v3Velocity * h / 2 ;
    Status.v3Velocity = Status0->v3Velocity + K1 * h / 2 ;
    Status.V3DAMP = Status0->V3DAMP;
    K2 = Second_Order_Defferential( &Status , Time + h / 2 , h );
    // 
    Status.v3Position = Status.v3Position + h * h * K1 / 4 ;
    Status.v3Velocity = Status0->v3Velocity + K2 * h / 2 ;
    K3 = Second_Order_Defferential( &Status , Time + h / 2 , h );
    // 
    Status.v3Position = Status0->v3Position + h * Status0->v3Velocity + K2 * h * h / 2 ;
    Status.v3Velocity = Status0->v3Velocity + K3 * h ;
    K4 = Second_Order_Defferential( &Status , Time + h , h );
    // 计算新的状态
    Status_Return.Time     = Time + h;
    ...
    Status_Return.v3Acceleration = V3ACCERLATION;
    // 
