...
CRENDEROBJECT           g_RenderObject_Coordinate;         // 显示的物体
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    lstrcat( szRcPath ,L"\\ResFile_1_Coordinate" );
    SetCurrentDirectory( szRcPath );
    TCHAR* szFile_1 = L"ShmFile.shm";
    if( FAILED( hr = g_RenderObject_Coordinate.OnCreateDevice( pd3dDevice ,szFile_1 ))) return hr;
    //
    lstrcpy( szRcPath ,szAppPath );
...
HRESULT CALLBACK OnResetDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    //
    g_RenderObject_Coordinate.OnResetDevice( pd3dDevice ,fAspectRatio );
...
void CALLBACK OnFrameMove( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    g_RenderObject_Coordinate.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime  );
    g_RenderObject_Ground.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime  );
    // 将物体渲染到纹理
    vEyePt.x  = vEyePt.x / 3 ;
    vEyePt.y  = vEyePt.y / 3 ;
    g_RenderObject_Coordinate.RenderToTexture( pd3dDevice , vEyePt ,-fTime );
    g_RenderObject_Ground.RenderToTexture( pd3dDevice , vEyePt ,fTime );
  }
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
      {
        g_RenderObject_Coordinate.OnFrameRender( pd3dDevice );
        g_RenderObject_Ground.OnFrameRender( pd3dDevice );
        /*
        g_TextureDebug.OnFrameRender( pd3dDevice ,g_RenderObject_Coordinate.ReturnTexture( ) , 0 );
        */
        
        g_TextureDebug.OnFrameRender( pd3dDevice ,g_RenderObject_Coordinate.ReturnTexture( ) , 1 );
        g_TextureDebug.OnFrameRender( pd3dDevice ,g_RenderObject_Ground.ReturnTexture( ) , 2 );
        g_TextureDebug.OnFrameRender( pd3dDevice ,g_RenderObject_Coordinate.ReturnTexture( ) , 3 );
        g_TextureDebug.OnFrameRender( pd3dDevice ,g_RenderObject_Ground.ReturnTexture( ) , 4 );
...
void CALLBACK OnLostDevice()
  {
    ...
    g_RenderObject_Coordinate.OnLostDevice( );
...
void CALLBACK OnDestroyDevice()
  {
    ...
    g_RenderObject_Coordinate.OnDestroyDevice( );
