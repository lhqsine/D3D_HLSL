...
void DXUTSetCallbackMouse( LPDXUTCALLBACKMOUSE pCallbackMouse, bool bIncludeMouseMove )   
  { GetDXUTState().SetMouseFunc( pCallbackMouse ); GetDXUTState().SetNotifyOnMouseMove( bIncludeMouseMove );   }
    ...
    if( DXUTGetHWNDFocus() == NULL )
      {
        ...
        /*
        DWORD dwWindowStyle = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX;
        */
        ...
        int iMax_X = GetSystemMetrics( SM_CXSCREEN );
        int iMax_Y = GetSystemMetrics( SM_CYSCREEN );
        int iXposition = ( iMax_X - ( rc.right-rc.left )) / 2;
        int iYposition = ( iMax_Y - ( rc.bottom-rc.top )) / 2;
        HWND hWnd = CreateWindow( L"Direct3DWindowClass", strWindowTitle, dwWindowStyle,
          iXposition, iYposition, (rc.right-rc.left), (rc.bottom-rc.top), 0,
          hMenu, hInstance, 0 );
        if( hWnd == NULL )
          {
            DWORD dwError = GetLastError();
...
void DXUTBuildOptimalDeviceSettings( DXUTDeviceSettings* pOptimalDeviceSettings, 
  DXUTDeviceSettings* pDeviceSettingsIn, DXUTMatchOptions* pMatchOptions )
  {
    IDirect3D9* pD3D = DXUTGetD3DObject();
    D3DDISPLAYMODE adapterDesktopDisplayMode;
...
float DXUTRankDeviceCombo( CD3DEnumDeviceSettingsCombo* pDeviceSettingsCombo, 
  DXUTDeviceSettings* pOptimalDeviceSettings, D3DDISPLAYMODE* pAdapterDesktopDisplayMode )
  {
    ...
    if( pNewDeviceSettings->pp.Windowed )
      {
        ...
        SetWindowPos( DXUTGetHWND(), HWND_NOTOPMOST,
          ptClient.x, ptClient.y,( rcWindowBounds.right - rcWindowBounds.left ),
          ( rcWindowBounds.bottom - rcWindowBounds.top ),  0 );
        ...
        SetWindowPos( DXUTGetHWND(), HWND_NOTOPMOST, 
          ptClient.x, ptClient.y,  0, 0, SWP_NOSIZE|SWP_SHOWWINDOW );
    ...
    switch( uMsg )
      {
        case WM_PAINT:
          {
            ...
            if( pd3dDevice && DXUTIsRenderingPaused() && 
              ...
              {
                //----------------------------------------------------
                // 这是 Splash 窗口需要的显示，可以删除
                /*
                LOGFONT      hLogFont;
                lstrcpy( hLogFont.lfFaceName,L"宋体" ); 
                hLogFont.lfWeight     = 700;                 // 粗细
                hLogFont.lfHeight     = -60;                 // 字高
                hLogFont.lfUnderline  = 0;                   // 下画线
                hLogFont.lfItalic     = 0;                   // 倾斜
                hLogFont.lfCharSet    = GB2312_CHARSET;// ; OEM_CHARSET    // 默认字符集
                HFONT hFont = CreateFontIndirect( &hLogFont );
        ...
        default:
          ...
          if( CDXUTIMEEditBox::StaticMsgProc( uMsg, wParam, lParam ) )              return 0;
      }
    ...
    switch( hr )
      {
        case DXUTERR_NODIRECT3D:             
          nExitCode = 2; 
          wcsncpy( strBuffer, L"Could not initialize Direct3D. You may want to check that the latest version of DirectX is correctly installed on your system.  Also make sure that this program was compiled with header files that match the installed DirectX DLLs.", 512 ); break;
        case DXUTERR_INCORRECTVERSION:       
          nExitCode = 10; 
          wcsncpy( strBuffer, L"Incorrect version of Direct3D and/or D3DX.", 512 ); break;
        case DXUTERR_MEDIANOTFOUND:          
          nExitCode = 4; 
          wcsncpy( strBuffer, L"Could not find required media. Ensure that the DirectX SDK is correctly installed.", 512 ); break;
        case DXUTERR_NONZEROREFCOUNT:        
          nExitCode = 5; 
          wcsncpy( strBuffer, L"The D3D device has a non-zero reference count, meaning some objects were not released.", 512 ); break;
        case DXUTERR_CREATINGDEVICE:         
          nExitCode = 6; 
          wcsncpy( strBuffer, L"Failed creating the Direct3D device.", 512 ); break;
        case DXUTERR_RESETTINGDEVICE:        
          nExitCode = 7; 
          wcsncpy( strBuffer, L"Failed resetting the Direct3D device.", 512 ); break;
        case DXUTERR_CREATINGDEVICEOBJECTS:  
          nExitCode = 8; 
          wcsncpy( strBuffer, L"Failed creating Direct3D device objects.", 512 ); break;
        case DXUTERR_RESETTINGDEVICEOBJECTS: 
          nExitCode = 9; 
          wcsncpy( strBuffer, L"Failed resetting Direct3D device objects.", 512 ); break;
        case DXUTERR_SWITCHEDTOREF:          
          nExitCode = 0; 
          wcsncpy( strBuffer, L"Switching to the reference rasterizer,\na software device that implements the entire\nDirect3D feature set, but runs very slowly.", 512 ); break;
