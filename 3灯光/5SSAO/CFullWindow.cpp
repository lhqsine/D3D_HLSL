...
HRESULT CFULLWINDOW::OnResetDevice( IDirect3DDevice9* pd3dDevice )
  {
    ...
    hr = m_pEffect->SetTexture( "g_texNoiseMap" ,m_pColorTexture );
    return hr;
  }
...
void CFULLWINDOW::OnFrameRender( IDirect3DDevice9* pd3dDevice ,D3DXMATRIXA16* pWorld ,D3DXMATRIXA16* pView ,D3DXMATRIXA16* pProject ,double fTime )
  {
    ...
    UINT iPass, cPasses;
    // ------------------------- ScreenShader ---------------------------
    hr = m_pEffect->SetTechnique( "techSSAO" );
...
void CFULLWINDOW::OnFrameRender( IDirect3DDevice9* pd3dDevice ,LPDIRECT3DTEXTURE9 pTexture )
  {
    HRESULT hr;
    hr = m_pEffect->SetTexture( "g_texDepthTexture" ,pTexture );
    hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
    hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( FULLWINDOW_D3DVERTEX )); 
    hr = pd3dDevice->SetIndices( m_pIB ); 
    UINT iPass, cPasses;
...
void CFULLWINDOW::OnFrameRender( IDirect3DDevice9* pd3dDevice ,LPDIRECT3DTEXTURE9 pDepthTexture , 
  LPDIRECT3DTEXTURE9 pNormalTexture )
  {
    HRESULT hr;
    hr = m_pEffect->SetTechnique( "techSSAO" );
    hr = m_pEffect->SetTexture( "g_texDepthTexture" , pDepthTexture );
    hr = m_pEffect->SetTexture( "g_texNormalTexture" , pNormalTexture );
    hr = m_pEffect->SetTexture( "g_texNoiseMap" ,m_pColorTexture );
    hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
    hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( FULLWINDOW_D3DVERTEX )); 
    hr = pd3dDevice->SetIndices( m_pIB ); 
