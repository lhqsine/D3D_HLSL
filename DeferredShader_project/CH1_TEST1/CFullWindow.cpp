...
void CFULLWINDOW::OnFrameRender( IDirect3DDevice9* pd3dDevice ,LPDIRECT3DTEXTURE9 pTexture )
  {
    HRESULT hr;
    hr = m_pEffect->SetTexture( "g_texDepthTextureFront" ,pTexture );
    hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
    hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( FULLWINDOW_D3DVERTEX )); 
    hr = pd3dDevice->SetIndices( m_pIB ); 
    UINT iPass, cPasses;
...
void CFULLWINDOW::OnFrameRender( IDirect3DDevice9* pd3dDevice ,LPDIRECT3DTEXTURE9 pPositionTexture , 
  LPDIRECT3DTEXTURE9 pNormalTexture )
  {
    HRESULT hr;
    hr = m_pEffect->SetTexture( "g_texPositionTexture" , pPositionTexture );
...
void CFULLWINDOW::SetLightParament( D3DXVECTOR4* vPosition )
  {
    HRESULT hr;
