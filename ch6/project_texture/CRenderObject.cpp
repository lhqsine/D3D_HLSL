...
HRESULT CRENDEROBJECT::OnCreateDevice( IDirect3DDevice9* pd3dDevice ,TCHAR* FileName )
  {
    ...
    if( FAILED( hr = pMeshSysMemClone->CloneMesh( D3DXMESH_MANAGED, decl, pd3dDevice, &m_pMeshSysMemory ))) return hr;
     // ------------------ 注意在此必须计算切向量 ------------------------------
//    if( FAILED( hr = D3DXComputeTangent( m_pMeshSysMemory, 0, 0, 0, TRUE, NULL ))) return hr;
	//
...
//	DWORD dwFlag = D3DXSHADER_DEBUG | D3DXSHADER_SKIPOPTIMIZATION;
    ...
    //  D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, 
    //  D3DX_DEFAULT, D3DX_DEFAULT, 0, 
    //  NULL, NULL, &m_pNormalTexture ) )) return hr;
    //// 高度纹理
    //szFile = GetResourceName( ENU_TEXHEIGHTMAP );
    //if( FAILED( hr = D3DXCreateTextureFromFileEx( pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, 
    //  D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, 
    //  D3DX_DEFAULT, D3DX_DEFAULT, 0, 
    //  NULL, NULL, &m_pHeightTexture ) )) return hr;
...
HRESULT CRENDEROBJECT::OnResetDevice( )
  {
    ...
    //if( FAILED( hr = m_pEffect->SetTexture( "g_NormalTexture" ,m_pNormalTexture ))) return hr;
    //if( FAILED( hr = m_pEffect->SetTexture( "g_HeightTexture" ,m_pHeightTexture ))) return hr;
    return hr;
  }
...
HRESULT CRENDEROBJECT::SetProject( D3DXMATRIXA16* pWorld , D3DXVECTOR3* pPosition ,D3DXVECTOR3* pObject ,D3DXVECTOR3* vUp )
  {
    ...
    D3DXMatrixPerspectiveFovLH( &matProjectProject ,D3DX_PI/6 ,1.0 ,1.0 ,100.0 );
    D3DXMatrixLookAtLH( &matViewProject ,pPosition , pObject , vUp );
    hr = m_pEffect->SetMatrix( "g_matWorldProject",  pWorld );
    hr = m_pEffect->SetMatrix( "g_matViewProject",   &matViewProject );
    hr = m_pEffect->SetMatrix( "g_matProjectProject",&matProjectProject );
    return S_OK;
  }
