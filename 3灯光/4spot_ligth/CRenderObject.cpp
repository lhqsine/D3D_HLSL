...
void CRENDEROBJECT::OnFrameMove( D3DXMATRIXA16* pWorld ,D3DXMATRIXA16* pView ,D3DXMATRIXA16* pProject ,double fTime , 
  D3DXVECTOR4* p4vEye ,D3DXVECTOR4* p4vLookAt )
  {
    HRESULT hr = S_OK;
    hr = m_pEffect->SetMatrix( "g_matWorld", pWorld ) ;
    hr = m_pEffect->SetMatrix( "g_matView", pView ) ;
    hr = m_pEffect->SetMatrix( "g_matProject", pProject );
    hr = m_pEffect->SetFloat( "g_fTime", ( float ) fTime ) ;
    //D3DXVECTOR4 v4EypPt   = D3DXVECTOR4( p3vEye->x , p3vEye->y , p3vEye->z , 1.0  );
    //D3DXVECTOR4 p4vLookAt = D3DXVECTOR4( p3vLookAt->x , p3vLookAt->y , p3vLookAt->z , 1.0  );
    hr = m_pEffect->SetVector( "g_vLightPosition", p4vEye );
    hr = m_pEffect->SetVector( "g_vLightLookAt", p4vLookAt );
    float Determinant;
    D3DXMATRIX matOut;
    D3DXMatrixInverse( &matOut , &Determinant ,pWorld );
    D3DXMatrixTranspose( &matOut ,&matOut );
...
void CRENDEROBJECT::SetLightParament( D3DXVECTOR4* vpPosition ,D3DXVECTOR4* vpDirect )
  {
    HRESULT hr;
    hr = m_pEffect->SetVector( "g_vLightPosition",vpPosition );
    hr = m_pEffect->SetVector( "g_vLightDirect",  vpDirect );
