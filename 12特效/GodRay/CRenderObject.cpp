...
HRESULT CRENDEROBJECT::OnCreateDevice( IDirect3DDevice9* pd3dDevice ,TCHAR* FileName )
  {
    ...
    if( FAILED( hr = D3DXCreateTextureFromFileEx( pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, 
      D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, 
      D3DX_DEFAULT, D3DX_DEFAULT, 0, 
      NULL, NULL, &m_pColorBlack ) )) return hr;
    // 
    return hr;
  }
...
HRESULT CRENDEROBJECT::OnResetDevice( IDirect3DDevice9* pd3dDevice )
  {
    ...
    if( FAILED( hr = m_pEffect->SetTexture( "g_BlackTexture" , m_pColorBlack ))) return hr;
    return hr;
  }

void CRENDEROBJECT::OnFrameMove( D3DXMATRIXA16* pWorld , D3DXMATRIXA16* pView , D3DXMATRIXA16* pProject , float fTime )
  {
    HRESULT hr = S_OK;
...
void CRENDEROBJECT::OnFrameRenderBG( IDirect3DDevice9* pd3dDevice )
  {
    HRESULT hr = S_OK;
    //
    hr = m_pEffect->SetTechnique( "DefaultTechBackGround" );
    hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
    hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( RENDEROBJECT_D3DVERTEX )); 
    hr = pd3dDevice->SetIndices( m_pIB ); 
    //
