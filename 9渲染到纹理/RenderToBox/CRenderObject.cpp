...
HRESULT CRENDEROBJECT::OnResetDevice( IDirect3DDevice9* pd3dDevice ,float fAspectRatio )
  {
    ...
    //hr = m_pRTTTextureBox->GetSurfaceLevel( 0, &m_pRTTSurfaceBox );
    /*
    hr = m_pRTTSurfaceBox->GetLevelDesc( 0 , &desc );
    if( FAILED( hr = D3DXCreateRenderToEnvMap( pd3dDevice , desc.Width ,
      desc.Format , TRUE , D3DFMT_D16 , &m_pRTTInterfaceBox )))
      {
        return hr;
      }
    */
...
HRESULT CRENDEROBJECT::RenderToTexture( LPDIRECT3DDEVICE9 pd3dDevice ,D3DXVECTOR3 vPosition ,float fTime )
  {
    ...
    if( SUCCEEDED( hr = m_pRTTInterfaceBox->BeginScene( m_pRTTSurfaceBox, NULL )))
      {
        hr = pd3dDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1.0f, 0 );
        hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
        hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( RENDEROBJECT_D3DVERTEX )); 
        ...
        hr = m_pEffect->End();
        hr = m_pRTTInterfaceBox->EndScene( 0 );
        // 
      }
    return hr;
  }
...
HRESULT CRENDEROBJECT::SetTexture( IDirect3DCubeTexture9* pCubeTexture ) 
  {
    HRESULT hr;
    hr = m_pEffect->SetTexture( "g_CubeTexture",pCubeTexture  );
    return hr;
  }
