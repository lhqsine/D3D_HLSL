...
HRESULT CBOXOBJECT::OnCreateDevice( IDirect3DDevice9* pd3dDevice ,TCHAR* FileName )
  {
    ...
    if( FAILED( hr = D3DXCreateCubeTextureFromFile( pd3dDevice, szFile , &m_pCubeTexuture ))) return hr;
    //
    return hr;
  }
...
HRESULT CBOXOBJECT::OnResetDevice( )
  {
    ...
    if( FAILED( hr = m_pEffect->SetTexture( "g_CubeTexture", m_pCubeTexuture ))) return hr;
    return hr;
  }

void CBOXOBJECT::OnFrameMove( D3DXMATRIXA16* pWorld ,D3DXMATRIXA16* pView ,D3DXMATRIXA16* pProject ,double fTime )
  {
    HRESULT hr = S_OK;
    ...
    hr = m_pEffect->SetMatrix( "g_matWorldNormalInverse", &matOut );
  }
...
void CBOXOBJECT::OnFrameRender( IDirect3DDevice9* pd3dDevice )
  {
    ...
    hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( CBOXOBJECT_D3DVERTEX )); 
