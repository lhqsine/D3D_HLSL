...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    // 
    lstrcpy( szRcPath ,szAppPath );
    lstrcat( szRcPath ,L"\\ResFile_3" );
    SetCurrentDirectory( szRcPath );
    TCHAR* szFile_3 = L"ShmFile.shm";
    if( FAILED( hr = g_Render_3.OnCreateDevice( pd3dDevice ,szFile_3 ))) return hr;
    // 
    return S_OK;
  }
...
void CALLBACK OnFrameMove( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    //
    g_Render_3.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime ,&vViewPosition );
    //
    ...
    g_RenderLight_2.OnFrameMove( &g_matWorld ,&g_matView ,&g_matProject ,fTime ,&vViewPosition );
  }
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
      {
        ...
        g_Render_3.OnFrameRender( pd3dDevice );
        //
