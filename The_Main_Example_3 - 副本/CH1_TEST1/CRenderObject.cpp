...
HRESULT CRENDEROBJECT::OnCreateDevice( IDirect3DDevice9* pd3dDevice ,TCHAR* FileName )
 ...
 //   if( FAILED( hr = LoadMesh( pd3dDevice ,szFile ,&m_pMesh ))) return hr;
	if( FAILED( hr = LoadMesh( pd3dDevice ,L"Model2.X" ,&m_pMesh ))) return hr;
    D3DXVECTOR3* pData;
...
//    if( FAILED( hr = D3DXCreateEffectFromFile( pd3dDevice, szFile, NULL, NULL, 0, NULL, &m_pEffect, &pCode )))
	  if( FAILED( hr = D3DXCreateEffectFromFile( pd3dDevice, L"FileFX.fx", NULL, NULL, 0, NULL, &m_pEffect, &pCode )))
...
*/
 /*   if( FAILED( hr = D3DXCreateTextureFromFileEx( pd3dDevice, L"ColorMap.JPG", D3DX_DEFAULT, D3DX_DEFAULT, 
      D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, 
      D3DX_DEFAULT, D3DX_DEFAULT, 0, 
      NULL, NULL, &m_pColorTexture ) )) return hr;
	  */
