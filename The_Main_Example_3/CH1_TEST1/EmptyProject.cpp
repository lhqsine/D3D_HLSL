...
CRENDEROBJECT           g_RenderObject;         // 显示的物体
//--------------------------------------------------------------------------------------
// 与用户交互的控制 ID
//--------------------------------------------------------------------------------------
...
void    InitApp();
void    RenderText( double fTime ,float );
...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
  {
    ...
    DXUTCreateWindow( L"EmptyProject_DXUT_6" );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    g_Camera.SetRadius( g_RenderObject.GetRadius( ) * 2.0f, g_RenderObject.GetRadius( ) * 0.5f, g_RenderObject.GetRadius( ) * 6.0f );
...
void CALLBACK OnFrameMove( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
  {
    ...
    g_RenderObject.OnFrameMove( &mWorld ,&mView ,&mProj ,fTime );
...
void RenderText( double fTime ,float fElapsedTime )
  {
    ...
    wszOutput[1023] = L'\0';
    _snwprintf( wszOutput, 1024 ,L"时间：fTime %f ,fElapsedTime %f",fTime ,fElapsedTime );
