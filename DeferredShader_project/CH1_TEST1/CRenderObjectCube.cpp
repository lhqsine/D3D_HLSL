...
HRESULT CRENDEROBJECTCUBE::OnCreateDevice( IDirect3DDevice9* pd3dDevice ,TCHAR* FileName )
  {
    ...
    if( FAILED( hr = D3DXCreateTextureFromFileEx( pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, 
      ...
      NULL, NULL, &m_pColorTexture ) )) return hr;  // m_pCubeTexuture
      */
    //
    szFile = GetResourceName( ENU_CUBEMAP );
    if( FAILED( hr = D3DXCreateCubeTextureFromFile( pd3dDevice, szFile , &m_pCubeTexuture ))) return hr;
    /*
    if( FAILED( hr = D3DXCreateCubeTextureFromFileEx( pd3dDevice, szFile, D3DX_DEFAULT, D3DX_DEFAULT, 
      D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, 
      D3DX_DEFAULT, D3DX_DEFAULT, 0, 
      NULL, NULL, &m_pCubeTexuture ) )) return hr;
      */
    //
    return hr;
  }
