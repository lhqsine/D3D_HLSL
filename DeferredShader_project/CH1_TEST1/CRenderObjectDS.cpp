...
CRENDEROBJECTDS::CRENDEROBJECTDS( )
  {
    ...
    //
    //m_TextureSize  = RTTTEXTURESIZE;
    // 
    m_pRTTTexturePosition = NULL;
    m_pRTTInterfacePosition = NULL;
    m_pRTTSurfacePosition = NULL;
    // 
    m_pRTTTextureNormal = NULL;
    m_pRTTInterfaceNormal = NULL;
    m_pRTTSurfaceNormal = NULL;
    //
  }
...
CRENDEROBJECTDS::~CRENDEROBJECTDS( )
  {
    ...
    SAFE_RELEASE( m_pRTTInterfacePosition ) ;
    SAFE_RELEASE( m_pRTTSurfacePosition ) ;
    //
    SAFE_RELEASE( m_pRTTTextureNormal );
    SAFE_RELEASE( m_pRTTInterfaceNormal );
    SAFE_RELEASE( m_pRTTSurfaceNormal );
    //
  }
...
HRESULT CRENDEROBJECTDS::OnResetDevice( IDirect3DDevice9* pd3dDevice , const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
  {
    ...
    hr = m_pRTTTexturePosition->GetSurfaceLevel( 0 , &m_pRTTSurfacePosition );
    hr = m_pRTTSurfacePosition->GetDesc( &desc );
    if( FAILED( hr = D3DXCreateRenderToSurface( pd3dDevice,desc.Width , desc.Height,
      desc.Format , TRUE , D3DFMT_D16 , &m_pRTTInterfacePosition )))
      {
        return hr;
      }
    // ------------------- 法线纹理 -----------------------------
    ...
    if( FAILED( hr = D3DXCreateTexture( pd3dDevice , m_TextureSize , m_TextureSize , 1, 
      D3DUSAGE_RENDERTARGET , D3DFMT_A16B16G16R16F , D3DPOOL_DEFAULT , &m_pRTTTextureNormal )))
      {
        if( FAILED( hr = D3DXCreateTexture( pd3dDevice , m_TextureSize , m_TextureSize , 1 , 
          0 , D3DFMT_A8B8G8R8 , D3DPOOL_DEFAULT , &m_pRTTTextureNormal )))
          {
            return hr;
          }
      }
      */
    //D3DSURFACE_DESC desc;
    hr = m_pRTTTextureNormal->GetSurfaceLevel( 0 , &m_pRTTSurfaceNormal );
    hr = m_pRTTSurfaceNormal->GetDesc( &desc );
    if( FAILED( hr = D3DXCreateRenderToSurface( pd3dDevice , desc.Width , desc.Height ,
      desc.Format , TRUE , D3DFMT_D16 , &m_pRTTInterfaceNormal )))
      {
        return hr;
      }
    // 
...
void CRENDEROBJECTDS::OnFrameMove( D3DXMATRIXA16* pWorld ,D3DXMATRIXA16* pView ,D3DXMATRIXA16* pProject ,double fTime )
  {
    HRESULT hr = S_OK;
...
void  CRENDEROBJECTDS::RenderToTexture( LPDIRECT3DDEVICE9 pd3dDevice , D3DXMATRIXA16* pWorld ,D3DXMATRIXA16* pView ,D3DXMATRIXA16* pProject ,double fTime )
  {
    HRESULT hr;
    // 计算3个矩阵
    hr = m_pEffect->SetTechnique( "RenderPositionTech" );
    ...
    if( SUCCEEDED( hr = m_pRTTInterfacePosition->BeginScene( m_pRTTSurfacePosition, NULL )))
      {
        hr = pd3dDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1.0f, 0 );
        hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
        hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( RENDEROBJECT_D3DVERTEXDS )); 
        ...
        hr = m_pEffect->End();
        hr = m_pRTTInterfacePosition->EndScene( 0 );
        // 
      }
    // 渲染法向量到纹理
    hr = m_pEffect->SetTechnique( "RenderNormalTech" );
    if( SUCCEEDED( hr = m_pRTTInterfaceNormal->BeginScene( m_pRTTSurfaceNormal , NULL )))
      {
        hr = pd3dDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1.0f, 0 );
        hr = pd3dDevice->SetVertexDeclaration( m_pDecl );
        hr = pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof( RENDEROBJECT_D3DVERTEXDS )); 
        ...
        hr = m_pEffect->End();
        hr = m_pRTTInterfaceNormal->EndScene( 0 );
        // 
      }
    // 完成任务，可以返回
  }
