...
HRESULT CRENDEROBJECT::OnResetDevice( IDirect3DDevice9* pd3dDevice , int iWidth , int iHeight )
  {
    HRESULT hr = S_OK;
    ...
    // ------------------- 渲染到纹理 -----------------------------
    if( FAILED( hr = D3DXCreateTexture( pd3dDevice , iWidth , iHeight , 1, 
    ...
    //-------------------------- 投影矩阵 ------------------------------------------
    D3DXMatrixPerspectiveFovLH( &m_matProjectRTT , D3DX_PI/4 , 1 , 1.0f , 80.0f );
    return hr;
  }
